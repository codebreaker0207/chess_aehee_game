(function(){
  ...
  function createMatrix(w,h){
    const m=[]; for(let i=0;i<h;i++) m.push(new Array(w).fill(0)); return m;
  }
  ...
  function collide(arena,player){
    ...
    for(let y=0;y<m.length;y++) for(let x=0;x<m[y].length;x++){if(m[y][x]!==0){if(!arena[y+o.y] || arena[y+o.y][x+o.x]!==0) return true}} return false;
  }

  function sweep(){
    let rowCount = 0; outer: for(let y=arena.length-1;y>=0;y--){
  ...
  function playerReset(){
    ...
    next = randomPiece();
    if(collide(arena,player)){arena = createMatrix(cols,rows); player.score = 0; player.lines=0; player.level=1; gameOver = true; updateHUD(); setTimeout(()=>{alert('게임오버');},100);}
    updateNext();
  }
  ...
  function draw(){ctx.clearRect(0,0,canvas.width,canvas.height); for(let y=0;y<arena.length;y++){for(let x=0;x<arena[y].length;x++){const val=arena[y][x]; if(val){ctx.fillStyle=COLORS[val]; ctx.fillRect(x*scale,y*scale,scale-1,scale-1);}}} player.matrix.forEach((row,y)=>{row.forEach((val,x)=>{if(val){ctx.fillStyle=COLORS[val]; ctx.fillRect((x+player.pos.x)*scale,(y+player.pos.y)*scale,scale-1,scale-1)}})});}
  function updateNext(){nextCtx.clearRect(0,0,nextCanvas.width,nextCanvas.height); const size=next.matrix.length; const tile=Math.floor(nextCanvas.width/size); for(let y=0;y<size;y++) for(let x=0;x<size;x++){const v=next.matrix[y][x]; if(v){nextCtx.fillStyle=COLORS[v]; nextCtx.fillRect(x*tile,y*tile,tile-2,tile-2)}}}
  function updateHUD(){document.getElementById('score').innerText=player.score; document.getElementById('lines').innerText=player.lines; document.getElementById('level').innerText=player.level;}
  ...
  document.addEventListener('keydown',e=>{if(e.key==='ArrowLeft') playerMove(-1); else if(e.key==='ArrowRight') playerMove(1); else if(e.key==='ArrowDown') playerDrop(); else if(e.key==='ArrowUp') playerRotate(); else if(e.code==='Space'){while(!collide(arena,player)){player.pos.y++} player.pos.y--; merge(arena,player); sweep(); playerReset();}});
  ...
  document.getElementById('startBtn').addEventListener('click',()=>{arena=createMatrix(cols,rows); player.score=0; player.lines=0; player.level=1; dropInterval=1000; gameOver=false; paused=false; next=randomPiece(); playerReset(); updateHUD();});
  document.getElementById('pauseBtn').addEventListener('click',()=>{paused=!paused; document.getElementById('pauseBtn').innerText=paused?'재개':'일시정지';});
